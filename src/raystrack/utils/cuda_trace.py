...
# Zero uint8 buffers
@cuda.jit
def kernel_zero_u8(a):
    i = cuda.grid(1)
    if i < a.size:
        a[i] = 0

__all__ += ["kernel_zero_u8"]
...
# -----------------------------------------------------------------
# Hitmask variants: mark per-ray any-hit (1) or miss (0)
# -----------------------------------------------------------------

@cuda.jit
def kernel_trace_hitmask(orig, dirs, v0, e1, e2, norm, sid, out_hitmask):
    k = cuda.grid(1)
    if k >= orig.shape[0]:
        return
    o0,o1,o2 = orig[k,0], orig[k,1], orig[k,2]
    ...
    for i in range(v0.shape[0]):
        ...
        if 1e-6<t<best:
            best=t; hit=sid[i]
    out_hitmask[k] = 1 if hit>=0 else 0


@cuda.jit
def kernel_trace_bvh_hitmask(orig, dirs, v0, e1, e2, norm, sid,
                             bb_min, bb_max, left, right, start, cnt,
                             out_hitmask):
    k = cuda.grid(1)
    if k >= orig.shape[0]:
...
__all__ += ["kernel_trace_hitmask", "kernel_trace_bvh_hitmask"]

# -----------------------------------------------------------------
# On-GPU Tregenza binning: classify sky-visible rays into 145 bins
# -----------------------------------------------------------------

@cuda.jit(device=True, inline=True)
def _tregenza_patch_id(dx, dy, dz):
    # Below horizon -> no sky
    if dz <= 0.0:
        return -1
    # Elevation (deg) from horizon, azimuth (deg) 0..360
    el = math.asin(dz) * 57.29577951308232
    az = math.atan2(dy, dx) * 57.29577951308232
    if az < 0.0:
        az += 360.0
    # Ring boundaries (upper elevation) and azimuth counts
    # ring 0..7: [(0,12)->30, (12,24)->30, (24,36)->24, (36,48)->24,
    #            (48,60)->18, (60,72)->12, (72,84)->6, (84,90)->1]
    ring_hi = (12.0, 24.0, 36.0, 48.0, 60.0, 72.0, 84.0, 90.0)
    ring_n  = (30,   30,   24,   24,   18,   12,   6,    1)
    ring_start = (0, 30, 60, 84, 108, 126, 138, 144)
    ridx = -1
    for j in range(8):
        if el < ring_hi[j] or (j == 7 and abs(el - ring_hi[j]) < 1e-6):
            ridx = j
            break
    if ridx < 0:
        return -1
    ...
    if n_az == 1:
        return base
    w = 360.0 / n_az
    # Alternate half-sector offset every other ring (start from ring 1)
...
@cuda.jit
def kernel_bin_tregenza(dirs, hitmask, counts):
    k = cuda.grid(1)
    n = dirs.shape[0]
    if k >= n:
        return
    ...
    dy = dirs[k, 1]
    dz = dirs[k, 2]
    pid = _tregenza_patch_id(dx, dy, dz)
    if pid >= 0:
        cuda.atomic.add(counts, pid, 1)

__all__ += ["kernel_bin_tregenza"]
