...
# Additional variants that only compute a per-ray hit mask (any intersection)

@nb.njit(parallel=True, fastmath=True, cache=True)
def trace_cpu_hitmask(orig, dirs, v0, e1, e2, norm, sid, out_hitmask):
    """Mark per-ray any-hit against scene geometry (no receiver accumulation).

    Parameters
    ----------
    orig, dirs : float32[n,3]
        Ray origins and directions.
    v0, e1, e2, norm, sid : flattened receiver triangle data (see flatten_receivers).
    out_hitmask : uint8[n]
        Output set to 1 if the ray hits any triangle, else 0.
    """
    n_rays = orig.shape[0]
    for k in nb.prange(n_rays):
        o = orig[k]; d = dirs[k]
...
@nb.njit(parallel=True, fastmath=True, cache=True)
def trace_cpu_bvh_hitmask(orig, dirs,
                           v0, e1, e2, norm, sid,
                           bb_min, bb_max, left, right, start, count,
                           out_hitmask):
    """Same as trace_cpu_hitmask, but traverses a prebuilt BVH.

    BVH arrays are produced by raystrack.utils.bvh.build_bvh.
    """
    n_rays = orig.shape[0]
    for k in nb.prange(n_rays):
        o = orig[k]
...
__all__ += ["trace_cpu_hitmask", "trace_cpu_bvh_hitmask"]
